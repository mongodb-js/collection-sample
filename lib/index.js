...
const debug = require('debug')('mongodb-collection-sample');
const ReservoirSampler = require('./reservoir-sampler');
const NativeSampler = require('./native-sampler');
...
function getSampler(db, collectionName, opts, done) {
  db.admin().serverInfo(function(err, res) {
    ...
    const supported = semver.gte(res.version, '3.1.6');
...
module.exports = function(db, collectionName, opts) {
  const readable = es.readable(function() {
