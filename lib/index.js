 ...
 * @param {Function} done
 */
function getSampler(db, collection_name, opts, done) {
  getKernelVersion({
    db: db
    ...
    if (!supported) {
      return done(null, new ReservoirSampler(db, collection_name, opts));
    }
    return done(null, new NativeSampler(db, collection_name, opts));
  });
}
 ...
 * @api public
 */
module.exports = function(db, collection_name, opts) {
  var readable = es.readable(function() {
    getSampler(db, collection_name, opts, function(err, src) {
      if (err) {
