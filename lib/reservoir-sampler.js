...
const debug = require('debug')('mongodb-collection-sample:reservoir-sampler');

const RESERVOIR_SAMPLE_LIMIT = 10000;
const RESERVOIR_CHUNK_SIZE = 1000;
...
function reservoirStream(collection, size, opts) {
  ...
  const stream = es.through(
    ...
    function end() {
      ...
      const chunks = _chunk(reservoir, opts.chunkSize);
      // create cursors for chunks
      const cursors = chunks.map(function(ids) {
        const cursor = collection.find({ _id: { $in: ids }}, { promoteValues: opts.promoteValues, raw: opts.raw });
        if (opts.fields) {
