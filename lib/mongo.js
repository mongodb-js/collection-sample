 ...
 * via map/reduce to collect a representative uniform sample of documents
 * from a collection.
 *
 * Usage:
 *
 * ```javascript
 * // Returns 1 random document from the user collection.
 * db.users.sample()
 *
 * // Returns up to 4 random documents from the user collection.
 * db.users.sample(4)
 *
 * // Returns up to 3 random documents from the user collection where username
 * // starts with L.
 * db.users.sample(3), {username: /^L/})
 //
 * // Returns up to 2 random documents from the user collection where username
 * // starts with L and only the username field.
 * db.users.sample(2, {username: /^L/}, {username: 1})
 * ```
 *
 * @param {Number} [size] of the sample to take [default: `1`].
